...
public:
    ...
    {
        if (val.find("0") != std::string::npos && buttonState == false)
        {
            std::cout << "Button Pressed and LED is OFF \n";
            buttonState = true;
        }
        else if (val.find("1") != std::string::npos && buttonState == true)
        {
            std::cout << "Button Released and LED is ON \n";
            buttonState = false;
        }
    }
};

bool Display::buttonState = true;

int main()
{
    /* First File: button */
    ReadWrite button("/dev/dio3");
    /* Second File: led */
    ReadWrite led("/dev/dio0");
